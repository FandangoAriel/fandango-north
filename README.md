...
הגיליון הראשי של כל חווה **לא** מתעדכן מדיווח מלאי. כל דיווח נכתב לגיליון חדש באותה חוברת, בשם «שם החווה + התאריך» (למשל `בית העמק 17-09-2026`). דיווח נוסף באותו יום מתווסף מתחת. בכל כניסה למסך (או בלחיצה על «רענון מהגיליון») נקראים הפריטים מחדש מהטבלה הראשית: שורות חדשות, מחיקות ושינוי מיקום בגיליון יופיעו. במצב דמו זה לא זורם מהגיליון החי.

סדר התצוגה באפליקציה ניתן לגרירה; אם חיברתם גוגל הוא נשמר בטאב `סדר`. דיווח קודם נפתח ממסך דיווח המלאי («עריכת דיווח קודם»). בכניסה הראשונה כותבים שם; הוא נשמר בטלפון ובטאב `משתמשים`, ובפעם הבאה אין צורך להזדהות שוב.
...
השרת עולה ב־[http://127.0.0.1:43187](http://127.0.0.1:43187). בטלפון: כתיבת שם בכניסה הראשונה, בחירת חווה, ואז דיווח מלאי או צ'קליסט העמסה.
...
טאבים בגיליון: `משתמשים` (עמודה A, שמות שנכנסו מהאפליקציה), `העמסות` ליומן, `דיווחים` ליומן דיווחי מלאי ולהערות העמסה, `סדר` לסדר הגרירה, וגיליון יומי לכל דיווח מלאי בשם החווה והתאריך. טבלאות החוות הקיימות נשארות לקריאה בלבד מדיווח.
